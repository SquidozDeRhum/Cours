...
int main() {
    ...
    while (!collision) {
        ...
        usleep(CLOCK);  // Pause pour ralentir la boucle
    }
...
*/
void progresser(int lesX[], int lesY[], char direction, bool *collision, bool *pomme) {
    int queueX = lesX[TAILLE_DEPART + pommesMangees - 1];
    int queueY = lesY[TAILLE_DEPART + pommesMangees - 1];


    // Déplacer les segments du serpent (de la queue vers la tête)
    for (int i = TAILLE_DEPART - 1 + pommesMangees; i > 0; i--) {
        lesX[i] = lesX[i - 1];
        lesY[i] = lesY[i - 1];
    }
    ...
    if (direction == HAUT) {
        if (lesY[0] <2 && lesX[0] == LARGEUR / 2) { // Trou en haut
            lesY[0] = HAUTEUR - 2; // Réapparaître juste avant le bord du bas
        } else {
            lesY[0] -= 1;
        }
    } else if (direction == BAS) {
        if (lesY[0] == HAUTEUR - 1 && lesX[0] == LARGEUR / 2) { // Trou en bas
            lesY[0] = 1; // Réapparaître juste après le bord du haut
        } else {
            lesY[0] += 1;
        }
    } else if (direction == GAUCHE) {
        if (lesX[0] <2 && lesY[0] == HAUTEUR / 2) { // Trou à gauche
            lesX[0] = LARGEUR - 2; // Réapparaître juste avant le bord de droite
        } else {
            lesX[0] -= 1;
        }
    } else if (direction == DROITE) {
        if (lesX[0] == LARGEUR - 1 && lesY[0] == HAUTEUR / 2) { // Trou à droite
            lesX[0] = 1; // Réapparaître juste après le bord de gauche
        } else {
            lesX[0] += 1;
        }
    }
...
void initPlateau() {
    ...
    plato[ HAUTEUR / 2][0] = VIDE;              // Trou à gauche
    plato[ HAUTEUR / 2][LARGEUR - 1] = VIDE;    // Trou à droite
    plato[0][LARGEUR / 2] = VIDE; // Trou en haut
...
void ajouterPomme(){
    ...
    while (plato[pommey][pommex] == OBSTACLE){
        pommex = 1 + rand() % (LARGEUR - 2);
        pommey = 1 + rand() % (HAUTEUR - 2);
    } 
